...
# ---------------------------------------------------------------------------
# Fixtures - implemented with monkeypatch and mock
# ---------------------------------------------------------------------------


@pytest.fixture
def mock_api_service(monkeypatch):
    """Intercepts requests.get to return a mock user response."""

    def mock_get(url):
        # Extract user_id from URL: https://api.example.com/users/{user_id}
...
def mock_system_time(monkeypatch):
    """Intercepts datetime.now to return a fixed timestamp."""
    ...
    # Patch 'datetime' class in the module where it's used
    # Since it's imported as 'from datetime import datetime', we patch that reference.
    monkeypatch.setattr("11-04-pytest-monkeypatch-solution.datetime", MockDateTime)


@pytest.fixture
def mock_file_operations(monkeypatch):
    """Intercepts built-in open() to prevent disk access."""
    m = mock_open(read_data="mocked line 1\nmocked line 2\n")
