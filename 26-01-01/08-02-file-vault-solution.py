...
def read_vault(path, key):
    # Check key first
    if key != "SECRET":
        # Requirement: raise and suppress traceback context
        raise AccessDeniedError("Invalid Key") from None

    try:
        with open(path, "r") as f:
            return f.read()
    except FileNotFoundError as e:
        # Requirement: preserve original traceback for debugging
        raise VaultError("Vault not found") from e
    except (PermissionError, IsADirectoryError) as e:
        # Requirement: preserve original traceback for debugging
        # Note: Opening a directory with open() can raise IsADirectoryError
        raise AccessDeniedError("System restriction") from e
